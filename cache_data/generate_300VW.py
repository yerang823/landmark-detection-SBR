...
def load_video_dir(root, dirs, save_dir, save_name):
  ...
  for idx, cdir in enumerate(dirs):
    ...
    #print('cdir=',cdir)
    #print('all_frames, frame_path=',frame_path)
    #print('all_annots, annot_path=',frame_path)
    
    #assert len(all_frames) == len(all_annots), 'The length is not right for {} : {} vs {}'.format(cdir, len(all_frames), len(all_annots))
    try: assert len(all_frames) == len(all_annots), 'The length is not right for {} : {} vs {}'.format(cdir, len(all_frames), len(all_annots))
    except: break
...
def generate_300vw_list(root, save_dir):
  ...
  test_1_dirs = ['001', '002', '003', '004', '007']  
  train_dirs  = ['016', '017', '018', '019', '020']
  ...
  assert len(train_dirs) == 5, 'The length of train_dirs is not right : {}'.format( len(train_dirs) )
  assert len(test_1_dirs) == 5, 'The length of test_3_dirs is not right : {}'.format( len(test_1_dirs) )
...
if __name__ == '__main__':
  ...
  print ('This dir : {}, HOME : [{}] : {}'.format(this_dir, HOME_STR, '/app/landmark-detection/'))
  #path_300vw = osp.join(os.environ[HOME_STR], 'datasets', 'landmark-datasets', '300VW_Dataset_2015_12_14')
  path_300vw = osp.join('/app/landmark-detection/SBR2/datasets/landmark-datasets/300VW_Dataset_2015_12_14/')
